...
export class Agent {
   ...
   */
  private async handleChatUpdates(subscription: AsyncIterable<ChatUpdate>): Promise<void> {
    try {
  ...
  private async handleUserMessage(message: any): Promise<void> {
    ...
    const coreMessage: CoreMessage = {
      ...
      content: message.content.map((part: any) => {
        ...
        if (part.type === 'file') return { type: 'file', data: part.data, mimeType: part.mimeType };
        return part;
        ...
        if (msg.role === 'user') {
          ...
          return {
            ...
            content: userMsg.content.map((part: any) => {
              ...
              if (part.type === 'file') return { type: 'file' as const, data: part.data, mimeType: part.mimeType };
              return part;
            }) as CoreMessage['content'],
          };
      ...
      for await (const chunk of fullStream) {
        if (chunk.type === 'text-delta') {
          ...
          if (this.currentMessagePartIndex === 0) {
            ...
            const currentMessage = server.interface.chat.getActiveChat()?.messages.find((m: any) => m.id === messageId);
            if (currentMessage && currentMessage.content[this.currentMessagePartIndex]) {
              currentMessage.content[this.currentMessagePartIndex].text += chunk.textDelta;
              server.interface.chat.updateMessage(messageId, currentMessage.content);
            }
          }
  ...
  private async processParallelToolCallsContent(
    ...
    if (this.server?.interface.chat && this.activeChatId && !options?.syntheticCall) {
      for (const tc of toolCalls) {
