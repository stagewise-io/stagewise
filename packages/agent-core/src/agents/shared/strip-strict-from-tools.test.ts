...
// The helper operates on a structural `Record<string, unknown>` shape.
// Casting through `unknown` keeps the test inputs readable without
// pulling in the full tool factory machinery.
type AnyToolSet = Record<string, Record<string, unknown>>;

function toToolSet(input: Record<string, unknown>): Partial<AnyToolSet> {
  return input as unknown as Partial<AnyToolSet>;
}
