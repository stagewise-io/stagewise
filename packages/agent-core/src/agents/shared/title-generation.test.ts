...
describe('generateSimpleTitle', () => {
  ...
  it('returns the title from the first model when it succeeds', async () => {
    ...
    const promise = generateSimpleTitle(makeMessages(2), hm, 'agent-1');
  ...
  it('passes an abortSignal to generateText', async () => {
    ...
    const hm = makeMockHostModels();
    await generateSimpleTitle(makeMessages(2), hm, 'agent-1');
  ...
  it('aborts a hanging first model via the timeout and falls back', async () => {
  ...
  it('uses only the last 10 messages for context', async () => {
    ...
    await generateSimpleTitle(messages, hm, 'agent-1');
