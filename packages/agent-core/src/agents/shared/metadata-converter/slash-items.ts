/**
 * Slash-command resolution + telemetry redaction helpers used by the
 * metadata-converter pipeline. Migrated from
 * `apps/browser/src/backend/agents/shared/prompts/utils/metadata-converter/slash-items.ts`
 * in Phase 10.
 *
 * These utilities are intentionally package-internal: they resolve a
 * skill's on-disk content and render it as an XML-wrapped instruction
 * block, with zero host-specific coupling. The host remains responsible
 * only for discovering skills and passing the list in.
 */
...
import type { SkillDefinition, SkillDefinitionUI } from '../../../types/skills';
 ...
 *
 * The skill is looked up by `id` in the provided `skills` list and its
 * `contentPath` is used for disk resolution. This supports builtin,
 * workspace-skill, global-skill, and plugin-skill sources.
 */
