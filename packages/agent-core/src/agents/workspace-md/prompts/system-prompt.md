...
You must create or update a single file: `{mountPrefix}/{workspaceMdRelativePath}` (where `{mountPrefix}` is the workspace mount prefix described in the File System section above).
...
- You MUST write `{mountPrefix}/{workspaceMdRelativePath}` using the `write` tool with the mount-prefixed path.
