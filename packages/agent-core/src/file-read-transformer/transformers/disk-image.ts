...
import { readdir, stat, writeFile, unlink, rm } from '../../fs/index.js';
import type {
  FileTransformer,
  FileTransformResult,
  TransformerContext,
...
function parseDirectoryRecord(buf: Buffer, offset: number): DirRecord | null {
  ...
  if (len === undefined || len < 33 || offset + len > buf.length) return null;
