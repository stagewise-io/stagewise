...
export interface FileReadTransformerOptions {
  ...
  /** Logger. */
  logger: Logger;
  /** Host paths resolver — used to resolve `att/` attachment paths. */
  host: HostPaths;
  /** Agent instance ID. */
  agentId: string;
   ...
   * extension (incl. the leading dot, e.g. `'.textclip'`). Consulted
   * before the core built-in `TRANSFORMER_BY_EXT` table. Used by hosts
   * that own non-universal blob types (e.g. browser text-clips, DOM
   * captures) without forking core.
   */
...
async function runTransformer(
  ...
  // Host-supplied transformers take precedence over built-ins so a host
  // can override a default (rare) and register transformers for
  // host-specific blob types (e.g. `.textclip`). Checked before the
  // compound table because compounds are core-defined.
  let transformer: FileTransformer | undefined = ctx.extraTransformers?.[ext];

  // Check compound extensions next (e.g. `.tar.gz`).
  if (!transformer) {
...
function wrapResult(
  ...
  if (firstPart && firstPart.type === 'text') {
    ...
    for (let i = 1; i < result.parts.length; i++) {
      ...
      if (p) parts.push({ ...p } as TextPart | ImagePart | FilePart);
    }
  } else {
  ...
  if (lastPart && lastPart.type === 'text') {
    (lastPart as TextPart).text += closeXml;
  } else {
...
export { populatePathReferences } from './populate-path-references';
export {
  extractPathLinksFromMessage,
  extractReadFilePathsFromAssistantMessage,
  extractReadFileRequestsFromAssistantMessage,
  type ReadFileRequest,
} from './path-references';
export { resolveMountedPath } from './resolve-path';
export { hashPath, hashBuffer, hashFile, hashDirectory } from './hash';
/**
 * Reusable transformer for structured text blobs (used by the browser
 * host to register `.textclip` / `.swdomelement` via
 * `AgentHost.fileReadTransformers`). Hosts may register it under their
 * own extensions or implement their own transformer with the same
 * `FileTransformer` signature.
 */
...
export {
  ...
  getMaxReadChars,
  setMaxReadChars,
  setMaxReadCharsFromContextWindow,
  getMaxPreviewLines,
  setMaxPreviewLines,
  countLinesFittingBudget,
  truncateTextContent,
  baseMetadata,
} from './format-utils';
