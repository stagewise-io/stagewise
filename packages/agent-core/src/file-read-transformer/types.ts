 ...
 * entry point, the per-type transformers, and the cache layer.
 */
...
export interface TransformerContext {
   ...
   * extension (incl. the leading dot, e.g. `'.textclip'`). Looked up
   * before the core built-in `TRANSFORMER_BY_EXT` table so hosts can
   * register transformers for host-specific blob types without
   * forking core.
   */
