...
import { SymbolKind, type SymbolInfo } from '../../ast';
...
function extractSignature(
  ...
  if (body) {
    ...
    raw =
      sourceText.substring(node.startIndex, node.endIndex).split('\n')[0] ?? '';
  }
      ...
      case 'lexical_declaration': {
        ...
        for (const decl of child.namedChildren) {
          ...
          if (value) {
            ...
            sig = (
              source.substring(decl.startIndex, decl.endIndex).split('\n')[0] ??
              ''
            ).trim();
          }
          members.push({
