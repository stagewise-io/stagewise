 ...
 *
 * Supports:
 * - `att/<key>` — agent data-attachment blob (resolved via `host.agentAttachmentPath`)
 ...
 * @param host — Host paths resolver, used for `att/` attachment paths.
...
export function resolveMountedPath(
  ...
  if (mountedPath.startsWith('att/')) {
    ...
    if (!key) return null;
    return host.agentAttachmentPath(agentId, key);
  }
