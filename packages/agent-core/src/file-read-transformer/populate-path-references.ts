...
// Broad structural alias: the host narrows mention/environment types via its
// own `UserMessageMetadata` overlay (e.g. adding `tab` mentions). Accepting
// `AgentMessage<any, any>` keeps this helper usable from both core and host
// without forcing unsafe casts at call sites.
type AnyAgentMessage = AgentMessage<any, any>;
 ...
 */
function collectPathsFromUserMessage(message: AnyAgentMessage): string[] {
