...
/**
 * Discover workspace-scoped skills under `.stagewise/skills/` and
 * `.agents/skills/`. Dedupes by name; `.stagewise/` wins over
 * `.agents/`.
 */
export async function getSkills(workspacePath: string): Promise<Skill[]> {
  const stagewiseSkillsPath = resolve(workspacePath, '.stagewise', 'skills');
