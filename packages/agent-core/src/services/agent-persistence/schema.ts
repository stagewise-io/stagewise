...
/**
 * Stored model identifier. Kept as a plain string inside agent-core so
 * the persistence layer has no dependency on any particular provider
 * registry — the host narrows the string to `ModelId` at its own
 * boundaries.
 */
const modelId = customType<{ data: string; driverData: string }>({
  dataType() {
    return 'text';
  },
  toDriver(value) {
...
export const agentInstances = sqliteTable(
  'agentInstances',
  {
    ...
    history: _sqliteJson('history')
      .notNull()
      .$type<unknown[]>()
      .$defaultFn(() => [] as unknown[]),
    queuedMessages: _sqliteJson('queued_messages').notNull().$type<unknown[]>(),
