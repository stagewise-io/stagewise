 ...
 *      after downscaling.
 *
 * All transformations are performed in-process using `sharp` (libvips).
 *
...
import type { ProcessedImageCacheService } from './index';
 ...
 *
 * Safe to call from any async context. Never throws — failures are reported
 * via `{ ok: false, error }`.
 */
