/**
 * Skill definition types consumed by both the package and the host.
 *
 * Migrated from `apps/browser/src/shared/skills.ts` as part of Phase 10.
 * Ownership now lives in `agent-core` so that the slash-command pipeline
 * in `agents/shared/metadata-converter/slash-items.ts` can resolve skills
 * without a reverse dependency on the browser host.
 *
 ...
 * - `global` — user-level skills from `~/.stagewise/skills/` or
 *   `~/.agents/skills/`
...
export function toSkillDefinitionUI(cmd: SkillDefinition): SkillDefinitionUI {
  const { contentPath: _contentPath, ...ui } = cmd;
  return ui;
}
